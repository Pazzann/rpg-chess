...
export default class Rook extends Figure{
    ...
    get imageSrc(): string {
        return this._src + ((this.color === FigureColors.WHITE) ? "WhiteRook.png" : "BlackRook.png");
    }
    ...
    canMoveFigure(cells: Cell[][], target: Vector2): boolean {
        ...
        if (target.x == this.position.x) {
            ...
            for (let y = Math.min(this.position.y, target.y) + 1; y < Math.max(this.position.y, target.y); y++) {
